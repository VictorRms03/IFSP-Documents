...
public class Exercicioi2p2 {
    ...
    public static String prefixToInfix( String prefix ) throws IllegalArgumentException {
        ...
        String[] prefixSplit = prefix.split("\\s+");
        for(int i=prefixSplit.length-1; i>=0; i--){
            
            if(isOperand(prefixSplit[i])){
                
                stack.push(prefixSplit[i]);
                
            } else if (isOperator(prefixSplit[i])){
                
                try {
                   stack.push("( " + stack.pop() + " " + prefixSplit[i] + " " + stack.pop() + " )"); 
                } catch (Exception e) {
                    throw new IllegalArgumentException("Found a Operator where is expected a Operand");
                }
    ...
    public static String prefixToPostfix( String prefix ) throws IllegalArgumentException {
        ...
        for(int i=prefixSplit.length-1; i>=0; i--){
            ...
            if(isOperand(prefixSplit[i])){
                ...
                try{
                    stack.push(stack.pop() + " " + stack.pop() + " " + prefixSplit[i]);
                } catch(Exception e) {
                    throw new IllegalArgumentException("Found a Operator where is expected a Operand");
                }
        ...
        for(String str : postfix.split("\\s+")){
            ...
            if(isOperand(str)){
                ...
                try {    
                    ...
                    stack.push("( " + operand2 + " " + str + " " + operand1 + " )");
                } catch(Exception e) {
                    throw new IllegalArgumentException("Found a Operator where is expected a Operand");
                }
        ...
        for(String str : postfix.split("\\s+")){
            ...
            if(isOperand(str)){
                ...
                try {
                    ...
                    stack.push(str + " " + operand2 + " " + operand1);
                } catch (Exception e){
                    throw new IllegalArgumentException("Found a Operator where is expected a Operand");
                }
    ...
    public static String infixToPostfix( String infix ) throws IllegalArgumentException {
        ...
        for(String str : infix.split("\\s+")){
            ...
            if(isOperand(str)){
                ...
                while((!stack.isEmpty()) && (getPrecedence(str) <= getPrecedence(stack.peek()))){
