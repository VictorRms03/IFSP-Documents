...
public class Parser {
    ...
    public void parseArrayTypeDecl() throws IOException {
        ...
        try {
            ...
            Token tokenType = scanner.getToken();
            match( Symbol.identifier );
            idTable.add( tokenType, IdType.arrayTypeId);
    ...
    public void parseSubprogramDecls() throws IOException {
        ...
        while ( scanner.getSymbol().isSubprogramDeclStarter() ) { parseSubprogramDecl(); }
    ...
    public void parseFunctionDecl() throws IOException {
        ...
        try {
            ...
            Token tokenFunction = scanner.getToken();
            match( Symbol.identifier );
            
            idTable.add( tokenFunction, IdType.functionId );
            idTable.openScope();

            if( scanner.getSymbol() == Symbol.leftParen ) { parseFormalParameters(); }
            ...
            Token tokenFunction2 = scanner.getToken();
            match( Symbol.identifier );

            if( !tokenFunction.getText().equals(tokenFunction2.getText() ) ) {
                throw error( tokenFunction2.getPosition(), "Function name mismatch." );
            }
    ...
    public void parseParameterDecl() throws IOException {
        ...
        try {
            ...
            if( scanner.getSymbol() == Symbol.varRW ) { matchCurrentSymbol(); }
            
            Token tokenParameter = scanner.getToken();
            match( Symbol.identifier );
            idTable.add( tokenParameter, IdType.variableId );
    ...
    public void parseStatements() throws IOException, ParserException {
        ...
        while( scanner.getSymbol().isStmtStarter() ) { parseStatement(); }
    ...
    public void parseStatement() throws IOException, ParserException {
        ...
        if( scanner.getSymbol() == Symbol.identifier ) {
            ...
            if ( null == idTable.get( scanner.getToken() ) ) { parseAssignmentStmt(); }
    
            else switch ( idTable.get( scanner.getToken() ) ) {
                ...
                case procedureId:
                    ...
                    break;
                case constantId:
                    String errorMsg = "Identifier \""+ idToken + "\" cannot start a statement.";
                    throw error( idToken.getPosition(), errorMsg );
                default:
                    throw internalError( "Identifier \"" + scanner.getToken() + "\" has not been declared." );
                    
            }
            
        } else switch ( scanner.getSymbol() ) {
    ...
    public void parseReturnStmt() throws IOException {
        ...
        try {
            ...
            if ( scanner.getSymbol().isExprStarter() ) { parseExpression(); }
    ...
    public void parseSimpleExpr() throws IOException {
        ...
        if( scanner.getSymbol().isAddingOperator() ){ matchCurrentSymbol(); }
    ...
    public void parseConstValue() throws IOException {
        ...
        try {
            ...
            if( scanner.getSymbol().isLiteral() ) { parseLiteral(); }
            else if( scanner.getSymbol() == Symbol.identifier ) { matchCurrentSymbol(); }
            else { throw error("Invalid constant."); }
    ...
    public void parseFunctionCall() throws IOException {
        ...
        try {
            ...
            if ( scanner.getSymbol().isExprStarter() ) { parseActualParameters(); }
